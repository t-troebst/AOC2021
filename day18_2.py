...
def max_sum_magnitude(ns):
    return max(magnitude(reduce(add(x, y))) for x in ns for y in ns if x is not y)

def main():
    with open("day18_input.txt") as f:
        nums = list(map(eval, f))
        print(f"Maximum magnitude of sums is {max_sum_magnitude(nums)}.")
